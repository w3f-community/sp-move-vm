...
use crate::storage::event::{Event, EventHandler, EventWriter};
    ...
    B: Balances,
{
    ...
    pub fn delete_resource(
        ...
        if !self.bank.handle_delete_balance(&address, &tag, &ty, tp)? {
            self.data_access
                .delete(AccessPath::new(address, tag.access_vector()));
        }
        Ok(())
    }
    ...
    pub fn insert_resource(
        ...
        if !self
            .bank
            .handle_insert_balance(&address, &tag, &ty, tp, &value)?
        {
    ...
    pub fn publish_module(&self, id: ModuleId, blob: Vec<u8>) {
        self.data_access
            .insert(AccessPath::new(*id.address(), id.access_vector()), blob);
    }
}
